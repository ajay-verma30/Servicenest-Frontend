...
import { Row, Col, Card, Badge, Spinner, Container,Button,Alert, Form } from "react-bootstrap";
...
const [selectedTeam, setSelectedTeam] = useState("");
  ...
  const handleDisable = async (e) => {
    ...
    try {
      await axios.patch(
        `http://localhost:3000/user/${id}/disable`,
        {},
        {
          headers: { Authorization: `Bearer ${accessToken}` },
        }
  ...
  const fetchTeams = async () => {
    try {
      const res = await axios.get(`http://localhost:3000/teams/${orgId}/all`); 
...
const handleSaveTeam = async () => {
    if (!selectedTeam) {
      setActionMessage({ type: "warning", text: "Please select a team first" });
      return;
    }
    ...
    return (
      <div style={{ minHeight: "100vh", backgroundColor: "#f8f9fa" }}>
        ...
        <Row className="g-0" style={{ minHeight: "calc(100vh - 60px)" }}>
          ...
          <Col xs={10} md={10} className="d-flex justify-content-center align-items-center">
